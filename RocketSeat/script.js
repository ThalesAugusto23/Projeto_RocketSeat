...
const markdownToHTML = (text) => {
    const converter = new showdown.Converter() // Criação de um objeto JS
    return converter.makeHtml(text)
}

const pergutarIA = async (question, game, apiKey) => {
    ...
    const response = await fetch(geminiURL, {
        ...
        //stringify é uma função que torna um objeto JS em JSON
        body: JSON.stringify({
            contents,
            tools
        })
...
const enviarFormulario = async (event) => {
    event.preventDefault() /*Só vai enviar o formulário 1 vez*/
    ...
    try{
        /* Perguntar a IA*/
        const text = await pergutarIA (question, game, apiKey)
